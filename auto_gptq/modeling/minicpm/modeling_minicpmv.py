    ...
    def get_vllm_embedding(self, data):
        if 'vision_hidden_states' not in data:
            ...
            if all_pixel_values:
                ...
                max_patches = (self.config.image_size // self.config.patch_size) ** 2

                all_pixel_values = torch.nn.utils.rnn.pad_sequence(all_pixel_values, batch_first=True,
                                                                   padding_value=0.0)
                all_pixel_values = torch.nn.functional.pad(all_pixel_values, 
                    (0, 0, 0, self.config.image_size ** 2 // self.config.patch_size - all_pixel_values.size(1)), 'constant', 0.0)
