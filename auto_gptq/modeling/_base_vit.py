...
class BaseGPTQForVIT(nn.Module, PushToHubMixin):
            ...
            new_examples.append(
                {
                    ...
                    "image_bound": example["image_bound"],
                    "tgt_sizes": example["tgt_sizes"],
                    "labels": labels,
                }
            )
    ...
    def quantize(
        ...
        for i in range(len(layers)):
            ...
            for names in inside_layer_modules:
                ...
                for j in range(num_batches):
                    ...
                    if len(layer_input)<2:
                        layer(*layer_input,attention_mask=None)
                    else:
                        layer(*layer_input)
                for h in handles:
                    h.remove()
            ...
            for j in range(num_batches):
                ...
                if len(layer_input)<2:
                    layer_output = move_to_device(
                        layer(*layer_input,attention_mask=None)[0],
                        cur_layer_device if cache_examples_on_gpu else CPU,
                    )
                else:
                    layer_output = move_to_device(
    ...
    def from_quantized(
        ...
        **kwargs,
        ...
        from .minicpm.configuration_minicpm import MiniCPMConfig
        config = MiniCPMConfig.from_pretrained(
            model_name_or_path, trust_remote_code=True, **cached_file_kwargs
        )
        ...
        if not use_qigen:
            ...
            with ContextManagers(init_contexts):
                ...
                ignore_layers.append("llm.model")
                # print("layers-------",layers)
...
__all__ = ["BaseGPTQForVIT", "BaseQuantizeConfig"]
